...
| pharmacy_type | VARCHAR | 20 | NOT NULL | 药房类型（中药房|西药房） |
...
| drug_type | VARCHAR | 20 | NOT NULL | 药品类型（中药|西药|中成药） |
| use_purpose | TEXT | | | 药品用途 |
| usage_method | TEXT | | | 使用方式 |
...
#### 2.7.1 字典类型表（dict_types）

| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
| ---- | ---- | ---- | ---- | ---- |
| dict_type_id | UUID | | PRIMARY KEY | 字典类型ID |
...
- 主键索引：dict_type_id
- 唯一索引：dict_type_code
- 普通索引：dict_type_name, status, sort_order

#### 2.7.2 字典项表（dict_items）

| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
| ---- | ---- | ---- | ---- | ---- |
| dict_item_id | UUID | | PRIMARY KEY | 字典项ID |
| dict_type_id | UUID | | FOREIGN KEY REFERENCES dict_types(dict_type_id) | 字典类型ID |
...
**索引设计**：
- 主键索引：dict_item_id
- 复合唯一索引：(dict_type_id, dict_item_code) 联合唯一索引
- 普通索引：dict_type_id, dict_item_name, status, sort_order
...
erDiagram
    ...
    doctors ||--o{ dict_types : created_by
    doctors ||--o{ dict_types : updated_by
    doctors ||--o{ dict_items : created_by
    doctors ||--o{ dict_items : updated_by
    ...
    dict_types }o--o{ drugs : drug_type
    dict_types }o--o{ drugs : use_purpose
    dict_types }o--o{ drugs : usage_method
    dict_types }o--o{ pharmacies : pharmacy_type
```
