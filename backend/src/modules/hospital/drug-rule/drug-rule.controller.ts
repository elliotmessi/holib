...
export class DrugRuleController {
  ...
  @ApiResult({ type: [DrugPrescriptionRuleEntity], isPage: true })
  @Perm(permissions.LIST)
  async list(@Query() query: DrugRuleQueryDto): Promise<Pagination<DrugPrescriptionRuleEntity>> {
  ...
  @Perm(permissions.LIST)
  async getByDrug(@Param("drugId") drugId: number, @Query("activeOnly") activeOnly?: boolean): Promise<DrugPrescriptionRuleEntity[]> {
  ...
  async create(@Body(CreatorPipe) createDto: CreateDrugRuleDto): Promise<DrugPrescriptionRuleEntity> {
  ...
  async update(@Param("id") id: number, @Body(UpdaterPipe) updateDto: UpdateDrugRuleDto): Promise<void> {
