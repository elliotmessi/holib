...
export class InventoryTransactionController {
  ...
  @ApiResult({ type: [InventoryTransactionEntity], isPage: true })
  @Perm(permissions.LIST)
  async list(@Query() query: InventoryTransactionQueryDto): Promise<Pagination<InventoryTransactionEntity>> {
  ...
  @Perm(permissions.LIST)
  async getByReference(@Param("referenceId") referenceId: number, @Query("referenceType") referenceType?: string): Promise<InventoryTransactionEntity[]> {
  ...
  async getStats(@Query("startDate") startDate: string, @Query("endDate") endDate: string, @Query("pharmacyId") pharmacyId?: number): Promise<any> {
