...
export class PrescriptionController {
  ...
  @ApiResult({ type: [PrescriptionEntity], isPage: true })
  @Perm(permissions.LIST)
  async list(@Query() query: PrescriptionQueryDto): Promise<Pagination<PrescriptionEntity>> {
  ...
  async getStats(@Query("startDate") startDate: string, @Query("endDate") endDate: string, @Query("doctorId") doctorId?: number): Promise<any> {
  ...
  async create(@Body(CreatorPipe) createDto: CreatePrescriptionDto, @AuthUser("uid") uid: number): Promise<PrescriptionEntity> {
  ...
  async review(@Param("id") id: number, @Body() reviewDto: ReviewPrescriptionDto, @AuthUser("uid") uid: number): Promise<void> {
