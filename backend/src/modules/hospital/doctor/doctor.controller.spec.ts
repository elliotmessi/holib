...
describe("DoctorController", () => {
  ...
  describe("GET /doctors", () => {
    it("should return all doctors with pagination", async () => {
      const mockDoctors = new Pagination(
        [
    ...
    it("should return doctors with query parameters", async () => {
      const mockDoctors = new Pagination([{ id: 1, doctorCode: "DOC001", name: "张三", status: "active" } as DoctorEntity], {
        itemCount: 1,
        itemsPerPage: 10,
        totalItems: 1,
        totalPages: 1,
  ...
  describe("POST /doctors", () => {
    it("should create a new doctor", async () => {
      ...
      const response = await request(app.getHttpServer()).post("/doctors").send(createDto).expect(201)
  ...
  describe("PUT /doctors/:id", () => {
    it("should update doctor successfully", async () => {
      ...
      const response = await request(app.getHttpServer()).put("/doctors/1").send(updateDto).expect(200)
  ...
  describe("PUT /doctors/:id/password", () => {
    it("should change password successfully", async () => {
      ...
      const response = await request(app.getHttpServer()).put("/doctors/1/password").send(changePasswordDto).expect(200)
    ...
    it("should handle incorrect old password", async () => {
      ...
      doctorService.changePassword.mockRejectedValue(new BusinessException(ErrorEnum.PASSWORD_MISMATCH))

      await request(app.getHttpServer()).put("/doctors/1/password").send(changePasswordDto).expect(500)
    })
    ...
    it("should handle non-existent doctor", async () => {
      ...
      doctorService.changePassword.mockRejectedValue(new BusinessException(ErrorEnum.USER_NOT_FOUND))

      await request(app.getHttpServer()).put("/doctors/999/password").send(changePasswordDto).expect(500)
    })
  })
