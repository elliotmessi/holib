...
export class OnlineService {
   ...
   * 罗列在线用户列表
   */
  async listOnlineUser(value: string, page = 1, pageSize = 10): Promise<Pagination<OnlineUserInfo>> {
    const token = await AccessTokenEntity.findOne({
      where: { value },
      relations: ["user"],
      cache: true,
    ...
    return new Pagination(paginatedItems, {
      itemCount: total,
  ...
  async kickUser(tokenId: string, user: IAuthUser): Promise<void> {
    ...
    if (targetUid === rootUserId || targetUid === user.uid) throw new BusinessException(ErrorEnum.NOT_ALLOWED_TO_LOGOUT_USER)
